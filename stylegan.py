...
def main():
    ...
    g_all.load_state_dict(torch.load("style_gan.pt"))
    ...
    new_img = g_synth(w_2 * 0.5 + w_1 * 0.5)
    ...
    img1_r = cv.resize(img1.squeeze().permute(1, 2, 0).detach().numpy(), (400, 400))
    img1_r = cv.cvtColor(img1_r, cv.COLOR_BGR2RGB)

    img2_r = cv.resize(img2.squeeze().permute(1, 2, 0).detach().numpy(), (400, 400))
    img2_r = cv.cvtColor(img2_r, cv.COLOR_BGR2RGB)

    img3_r = cv.resize(new_img.squeeze().permute(1, 2, 0).detach().numpy(), (400, 400))
    img3_r = cv.cvtColor(img3_r, cv.COLOR_BGR2RGB)

    cv.imshow('img1', img1_r)
    cv.imshow('img2', img2_r)
    cv.imshow('img1 and img2 interpolation', img3_r)
